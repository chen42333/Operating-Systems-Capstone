...
void exception_entry()
{
    ...
    uart_write_string("SPSR_EL1: ", IO_ASYNC);
    uart_write_hex(value, sizeof(uint64_t), IO_ASYNC);
    uart_write_string("\r\n", IO_ASYNC);
    asm volatile ("mrs %0, elr_el1" : "=r"(value));
    uart_write_string("ELR_EL1: ", IO_ASYNC);
    uart_write_hex(value, sizeof(uint64_t), IO_ASYNC);
    uart_write_string("\r\n", IO_ASYNC);
    asm volatile ("mrs %0, esr_el1" : "=r"(value));
    uart_write_string("ESR_EL1: ", IO_ASYNC);
    uart_write_hex(value, sizeof(uint64_t), IO_ASYNC);
    uart_write_string("\r\n", IO_ASYNC);
}
...
void elasped_time()
{
    ...
    uart_write_dec(count / freq, IO_ASYNC);
